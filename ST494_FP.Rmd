...
lda_preds <- predict(model, newdata = test_cls)

lda_class <- preds$class
...
log_reg_model <- glm(gpa_letter ~ ., data = train_cls, family="binomial") # This is the method taught in class 

log_reg_pred_probs <- predict(model, newdata = train_cls, type = "probs")
log_reg_pred_class <- predict(model, newdata = test_cls, type = "class")


log_reg_accuracy <- mean(predicted_class == test_cls$gpa_letter)
