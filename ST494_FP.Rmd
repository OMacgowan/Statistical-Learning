...
df2 <- df |>
       ...
       select(-c("roll_no",
                 "science_score",
                 "writing_score"    #REMOVE FOR NOW(NOTE GPA is AV of all 4)!!!!!!!!!!!!!!!!!!!!!!!!!,
...
preds <- predict(linear_model, test_reg)
RMSE(test_y, preds)
...
model <- lda(gpa_letter ~ ., data = train_cls)

preds <- predict(model, newdata = test_cls)

class <- preds$class

table(class, test_cls$gpa_letter)


accuracy <- mean(class == test_cls$gpa_letter)
print(accuracy)
summary(lda_model)
...
model <- multinom(gpa_letter ~ ., data = train_cls)

predicted_probs <- predict(model, newdata = train_cls, type = "probs")
predicted_class <- predict(model, newdata = test_cls, type = "class")


accuracy <- mean(predicted_class == test_cls$gpa_letter)
...
linear_model <- svm(gpa_letter ~ .,data=train_cls, cost=best, kernel="linear")
mean(predict(linear_model, test_cls, type="class") != test_cls$gpa_letter)

best <- tune(svm, gpa_letter ~ ., data = train_cls, ranges = list(cost = try), kernel="radial")$best.parameters$cost
radial_model <- svm(gpa_letter ~ .,data=train_cls, cost=best, kernel="radial")
mean(predict(radial_model, test_cls, type="class") != test_cls$gpa_letter)

best <- tune(svm, gpa_letter ~ ., data = train_cls, ranges = list(cost = try), kernel="polynomial",degree=2)$best.parameters$cost
quadratic_model <- svm(gpa_letter ~ .,data=train_cls, cost=best, kernel="polynomial",degree=2)
mean(predict(quadratic_model, test_cls, type="class") != test_cls$gpa_letter)
